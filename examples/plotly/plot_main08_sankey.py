...
def build_and_plot(df_in: pd.DataFrame, out_png: str, title_suffix: str = "",
    ...
    pd.DataFrame(links, columns=["item_from", "day_from", "day_to", "item_to"]) \
        .groupby(["day_from", "item_from", "day_to", "item_to"], as_index=False) \
        .size().rename(columns={"size": "count"}).to_csv('./objects/plot_main08_sankey/flow.csv')
...
import pathlib # Path from matplotlib overrrides
